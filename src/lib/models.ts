...
export type ModelId = (typeof modelIds)[number] | (string & {});

export const modelsByProvider = modelIds.reduce((acc, modelId) => {
  const [provider, ...rest] = modelId.split("/");
  const model = rest.join("/");
  if (!acc[provider]) {
    acc[provider] = [];
  }
  acc[provider].push({ id: modelId, name: model });
  return acc;
}, {} as Record<string, { id: ModelId; name: string }[]>);

export const defaultModel = "openai/gpt-4.1-nano" as const;
